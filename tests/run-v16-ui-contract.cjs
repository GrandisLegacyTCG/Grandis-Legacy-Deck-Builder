...
need(pkg.version==='1.6.0','package version must be 1.6.0');
...
need(css.includes('Style 2 v2.14')&&css.includes('display:flex!important')&&css.includes('flex-flow:row wrap!important')&&css.includes('legacy-reference-card-tile')&&css.includes('aspect-ratio:5/7!important')&&css.includes('overflow-x:hidden!important'),'Legacy library cards must use a non-overlapping Main Library-scale grid');
need(css.includes('opacity:1!important')&&css.includes('filter:none!important'),'Reference cards must not be dimmed');
need(js.includes("builder_version:'2.14-classic-split'"),'Style 2 builder version not bumped to 2.14');
need(read(root+'/js/app.js').includes("builder_version:'3.13-public-deck-builder'"),'Style 1 v3.13 must remain unchanged');
console.log('PASS Deck Builder v1.6 UI contract');
