...
need(pkg.version==='1.3.0','package version must be 1.3.0');
...
need(css.includes('--legacy-reference-card-width')&&css.includes('grid-template-columns:repeat(auto-fill,var(--legacy-reference-card-width))')&&css.includes('height:auto!important'),'Legacy library cards must use a non-overlapping Main Library-scale grid');
need(css.includes('opacity:1!important')&&css.includes('filter:none!important'),'Reference cards must not be dimmed');
need(js.includes("builder_version:'2.11-classic-split'"),'Style 2 builder version not bumped');
need(read(root+'/js/app.js').includes("builder_version:'3.13-public-deck-builder'"),'Style 1 v3.13 must remain unchanged');
console.log('PASS Deck Builder v1.3 UI contract');
