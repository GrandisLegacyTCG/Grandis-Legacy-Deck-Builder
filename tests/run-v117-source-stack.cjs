...
for(const name of fs.readdirSync('starter_deck_examples').filter(file=>file.endsWith('.json'))){
  ...
  assert.equal(starter.builder_version,'1.26-public-deck-builder',`${name}: stale repository release metadata`);
...
console.log('PASS Deck Builder v1.26 corrected 198-card and Hero Component Source Stack parity');
