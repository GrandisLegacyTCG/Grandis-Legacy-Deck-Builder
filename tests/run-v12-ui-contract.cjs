...
need(pkg.version==='1.2.0','package version must be 1.2.0');
...
need(/legacyReferenceOpen[\s\S]{0,220}legacyDeckTotal/.test(s2),'Legacy Deck Library button must sit beside the 0/12 total');
...
need(css.includes('.legacy-reference-grid .library-card{')&&css.includes('width:100%!important')&&css.includes('minmax(190px,1fr)'),'Legacy cards must use readable Main Library-scale sizing');
need(css.includes('opacity:1!important')&&css.includes('filter:none!important'),'Reference cards must not be dimmed');
need(js.includes("builder_version:'2.10-classic-split'"),'Style 2 builder version not bumped');
need(read(root+'/js/app.js').includes("builder_version:'3.13-public-deck-builder'"),'Style 1 v3.13 must remain unchanged');
console.log('PASS Deck Builder v1.2 UI contract');
