...
for(const rel of ['js/data.js','style-2/js/data.js']){const d=load(rel);assert.equal(d.heroComponentRegistryHash,components.registry_hash);assert.equal(d.heroComponents.registry_hash,components.registry_hash);assert.equal(d.sourceStack.canonicalCardAuthority,'1.6.0');assert.equal(d.sourceStack.sharedRuntime,'1.94.0');assert.equal(d.sourceStack.effectRecipe,'0.15.0');assert.equal(d.sourceStack.effectCheckpoint,'0.15.0');assert.equal(d.sourceStack.starter60,'1.6.1');assert.equal(d.sourceStack.starterAuthoritySource,'1.9.5');assert.equal(d.sourceStack.currentAuthorityReference,'1.9.5');assert.equal(d.sourceStack.gameplayRuntimePropagation,'NOT_PERFORMED');assert.equal(d.sourceStack.sourceAuthority,'1.9.1');assert.equal(d.sourceStack.applicationRuntimeSync,'2.59');}
