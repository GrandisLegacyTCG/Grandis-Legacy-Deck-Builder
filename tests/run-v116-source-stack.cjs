...
for(const name of fs.readdirSync('starter_deck_examples').filter(file=>file.endsWith('.json'))){
  ...
  assert.equal(starter.builder_version,'1.16-public-deck-builder',`${name}: stale repository release metadata`);
...
console.log('PASS Deck Builder v1.16 corrected 198-card and Hero Component Source Stack parity');
