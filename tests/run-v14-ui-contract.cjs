...
need(pkg.version==='1.4.0','package version must be 1.4.0');
...
need(css.includes('Style 2 v2.12')&&css.includes('grid-template-columns:repeat(auto-fit,minmax(150px,1fr))')&&css.includes('overflow-x:hidden!important')&&css.includes('height:auto!important'),'Legacy library cards must use a non-overlapping Main Library-scale grid');
need(css.includes('opacity:1!important')&&css.includes('filter:none!important'),'Reference cards must not be dimmed');
need(js.includes("builder_version:'2.12-classic-split'"),'Style 2 builder version not bumped');
need(read(root+'/js/app.js').includes("builder_version:'3.13-public-deck-builder'"),'Style 1 v3.13 must remain unchanged');
console.log('PASS Deck Builder v1.4 UI contract');
