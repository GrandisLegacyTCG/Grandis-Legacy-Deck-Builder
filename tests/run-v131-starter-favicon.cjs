...
for(const f of files){const s=JSON.parse(fs.readFileSync(path.join(activeDir,f),'utf8'));assert.strictEqual(s.main_deck.reduce((n,x)=>n+Number(x.quantity||0),0),60,f);assert(String(s.format).includes('v1.9.1'),f);assert(String(s.format).includes('v1.6.0'),f);}
...
console.log('PASS Deck Builder v1.31 OSA v1.9.1 five-starter / favicon regression');
