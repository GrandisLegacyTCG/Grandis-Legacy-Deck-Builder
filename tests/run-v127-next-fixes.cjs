...
must(pkg.version==='1.27.0','package version');
must(s1.includes('function isLegalMainDeckSize(total){return total===50||total===60}'),'Style 1 50/60 missing');
must(s2.includes('const MAIN_DECK_LEGAL_SIZES=[50,60]'),'Style 2 50/60 missing');
...
console.log('PASS v1.27 next-fix contract');
