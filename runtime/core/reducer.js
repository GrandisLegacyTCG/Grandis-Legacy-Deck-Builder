...
function cardCost(card, state, playerId, sourceSlot) {
  ...
  if (cost && cost.kind === 'fixed') resolved = Number(cost.mana || 0);
    ...
    resolved = exact ? Number(exact[1] || 0) : Math.min(...Object.values(table).map(Number).filter(Number.isFinite));
...
function structuredClassResolutionOrder(state, pending, cls) {
  const order = [];
  const add = value => {
...
function classDamageFromStructuredData(state, pending, card, cls) {
  ...
  const classOrder = structuredClassResolutionOrder(state, pending, cls);
