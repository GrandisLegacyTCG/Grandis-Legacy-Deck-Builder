/* Grandis Legacy Browser Runtime Authority v1.73.
...
(function(global){
  ...
  function selectClassRow(map, hero, cardObj, ctx){
    map=map||{}; ctx=ctx||{}; var hc=String(ctx.heroClass||'');
    if(Object.prototype.hasOwnProperty.call(map,hc)) return hc;
    var lineage=activeLineage(hero,ctx.heroCard,hc).slice().reverse();
    for(var i=0;i<lineage.length;i++) if(Object.prototype.hasOwnProperty.call(map,lineage[i])) return lineage[i];
    return hc;
  }
  function mappedNumber(map, hero, cardObj, ctx){
    var key=selectClassRow(map,hero,cardObj,ctx), value=map&&map[key];
  ...
  function dynamicManaCost(card, sourceHero, state, sourceSide, ctx){
    ...
    if(heroClass && Object.prototype.hasOwnProperty.call(byClass,heroClass)) cost=Number(byClass[heroClass]);
      ...
      if(Object.prototype.hasOwnProperty.call(byClass,row)) cost=Number(byClass[row]);
    } else {
  ...
  global.GL_RULES_RUNTIME={version:'v1.4',authorityContract:{mode:'SINGLE_RUNTIME_AUTHORITY',clientRole:'INTENT_RENDER_ANIMATE_ONLY',releaseGate:'MANDATORY_SYNC_PARITY'},activeLineage:activeLineage,selectClassRow:selectClassRow,mappedNumber:mappedNumber,dynamicManaCost:dynamicManaCost,scalingDamage:scalingDamage,nextRankThreshold:nextRankThreshold,resolveTributeExp:resolveTributeExp,venomDetonationDamage:venomDetonationDamage};
