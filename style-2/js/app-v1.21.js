...
const MAIN_DECK_LEGAL_SIZES=[50,60];
const MAIN_DECK_LEGAL_LIMIT=60;
const MAIN_DECK_WORKSPACE_LIMIT=80;
function isLegalMainDeckSize(total){return MAIN_DECK_LEGAL_SIZES.includes(Number(total))}
...
function renderMainDeck(){
  ...
  const valid=validationIssues().length===0,wrongSize=!isLegalMainDeckSize(total);
  ...
  const exportButton=$('exportJson');if(exportButton){exportButton.disabled=wrongSize;exportButton.title=wrongSize?'Main Deck must contain exactly 50 or 60 cards before exporting.':''}
