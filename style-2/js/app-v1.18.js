...
function mainDeckViewCardHtml(card,quantity){return `<article class="main-deck-view-tile" title="${esc(card.name)}"><img src="${card.image}" alt="${esc(card.name)}"><span class="quantity-badge main-deck-view-quantity">×${quantity}</span>${reviewButton(card.id)}</article>`}
