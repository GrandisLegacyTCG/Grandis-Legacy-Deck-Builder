...
const DECK_BUILDER_RELEASE='1.17';
...
function libraryCardHtml(card){const compatible=cardCompatible(card),quantity=Number(state.deck[card.id]||0),canDrag=compatible&&!isMobileMainDeck()&&quantity<copyLimit(card)&&countDeck()<MAIN_DECK_WORKSPACE_LIMIT;return `<article class="library-card ${compatible?'':'incompatible'}" data-library-card="${card.id}" draggable="${canDrag?'true':'false'}" title="${esc(card.name)}${compatible?'':' — Not compatible'}"><img src="${card.image}" alt="${esc(card.name)}">${quantity>0?`<span class="quantity-badge library-quantity-badge">×${quantity}</span>`:''}${reviewButton(card.id)}</article>`}
...
function renderMainDeck(){
  ...
  if(mobile){
    ...
    root.innerHTML=entries.map(({card,quantity})=>`<article class="deck-card" data-deck-card="${card.id}" draggable="true" title="${esc(card.name)}"><img src="${card.image}" alt="${esc(card.name)}"><span class="quantity-badge">×${quantity}</span>${reviewButton(card.id)}</article>`).join('');
  ...
  root.scrollTop=scrollTop;bindReviewButtons(root);
  ...
  const valid=validationIssues().length===0,wrongSize=total!==MAIN_DECK_LEGAL_LIMIT;
  ...
  const exportButton=$('exportJson');if(exportButton){exportButton.disabled=wrongSize;exportButton.title=wrongSize?`Main Deck must contain exactly ${MAIN_DECK_LEGAL_LIMIT} cards before exporting.`:''}
...
async function exportDeck(){if(countDeck()!==MAIN_DECK_LEGAL_LIMIT){toast(`Main Deck must contain exactly ${MAIN_DECK_LEGAL_LIMIT} cards before exporting`);return}const object=exportObject();if(!object.is_valid){const ok=await askConfirm({eyebrow:'EXPORT DECK',title:'Export an incomplete deck?',message:'This deck is not match-ready yet.',list:object.validation_issues.map(issue=>({label:issue,value:'Issue'})),okLabel:'EXPORT ANYWAY'});if(!ok)return}downloadJson(object);toast('Deck exported')}
